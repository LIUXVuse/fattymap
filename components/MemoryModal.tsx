...
export const MemoryModal: React.FC<MemoryModalProps> = ({
    ...
    useEffect(() => {
        if (initialData) {
            // 編輯模式：優先使用已儲存的 identityType
            if (initialData.identityType) {
                // 如果有明確儲存的身分類型，直接使用
                setIdentityType(initialData.identityType);
                if (initialData.identityType === 'custom') {
                    setCustomName(initialData.author);
                    if (initialData.authorAvatar) setCustomAvatarPreview(initialData.authorAvatar);
                }
            } else if (initialData.isAnonymous) {
                // 向下相容：舊資料沒有 identityType，根據 isAnonymous 判斷
                setIdentityType('anonymous');
            } else {
                // 向下相容：嘗試判斷是 Google 還是自訂
                // 只有當用戶目前登入的 Google 帳號與留言的 creatorId 相同時才判斷為 Google 身分
                const isGoogleIdentity = currentUser &&
                    initialData.creatorId === currentUser.uid &&
                    initialData.author === currentUser.displayName;
                                    ...
                                    {recentIcons.map((iconKey) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`flex items-center justify-center w-9 h-9 rounded-lg border transition-all ${markerIcon === iconKey
                                    ...
                                    <div className="w-24 bg-gray-50 border-r border-gray-200 overflow-y-auto">
                                        {Object.keys(ICON_CATEGORIES).map(cat => (
                                            <button
                                                ...
                                                className={`w-full text-left px-3 py-3 text-xs font-bold border-l-4 transition-colors ${selectedIconCategory === cat
                                            ...
                                            {ICON_CATEGORIES[selectedIconCategory].map(iconKey => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        type="button"
                                                        onClick={() => handleIconSelect(iconKey)}
                                                        className={`aspect-square rounded-xl flex flex-col items-center justify-center gap-1 border transition-all ${markerIcon === iconKey
